...
public class OrderController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<OrderResponse> update(@PathVariable Integer id, @RequestBody OrderRequest r){
        return ResponseEntity.ok(OrderMapper.toResponse(oService.update(id, r)));
    }
    ...
    @GetMapping("/orders/by-establishment/{id}/status?={status}")
    ...
    @GetMapping("/orders/by-client/{id}/status?={status}")
    public ResponseEntity<List<OrderResponse>> findByClientAndStatus(@PathVariable Integer id, @PathVariable Integer status){
        return ResponseEntity.ok(OrderMapper.toResponse(oService.findByClientAndStatus(id, status)));
    }
