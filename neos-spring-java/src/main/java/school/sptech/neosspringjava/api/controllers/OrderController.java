...
public class OrderController {
    ...
    // findAllOrdersByEstablishment(@PathVariable Integer id){
    // return
    // ResponseEntity.ok(OrderMapper.toResponse(oService.findAllOrdersByEstablishment(id)));
    // }

    @GetMapping("/clientId/{id}")
    public ResponseEntity<List<OrderResponse>> findAllOrdersByClient(@PathVariable Integer id) {
        List<Order> listOrder = oService.findAllOrdersByClient(id);
        if (listOrder == null || listOrder.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(OrderMapper.toResponse(listOrder));
    }
    ...
    public ResponseEntity<List<OrderResponse>> findOrdersByClientIdAndStatusId(@PathVariable Integer id,
    ...
    // public ResponseEntity<List<OrderResponse>>
    // findByEstablishmentAndStatus(@PathVariable Integer id, @PathVariable Integer
    // status){
    // return
    // ResponseEntity.ok(OrderMapper.toResponse(oService.findByEstablishmentAndStatus(id,
    // status)));
    // }
